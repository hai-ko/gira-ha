...
class GiraX1DataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def _determine_callback_base_url(self) -> str:
        ...
        try:
            ...
            # Check for HTTPS proxy configuration as priority fallback
            https_proxy_url = "https://home.hf17-1.de"
            if https_proxy_url:
                _LOGGER.info("🌐 CALLBACK URL: Using HTTPS proxy - %s", https_proxy_url)
                _LOGGER.info("💡 HTTPS PROXY: This should resolve SSL/TLS connectivity issues with Gira X1")
